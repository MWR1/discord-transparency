...
const scriptURL = "https://api.github.com/repos/MWR1/discord-transparency/contents/startup-with-discord.js";
...
async function applyScript(repositoryScript) {
  try {
    ...
    await writeToFile(`${transparencyUnloadPath}\\__unpacked\\app\\mainScreen.js`, alteredSourceCode);
...
fetch(scriptURL)
  ...
  .then((response) => {
    ...
    if (currentVersion === repositoryVersion) {
      ...
      mainHeading.textContent = !currentVersion ? "Install the script for the first time!" : "Update found!";
      mainUpdateButton.textContent = !currentVersion ? "Install" : "Update";

      return scriptApplier(!currentVersion ? "install" : "update", response.content, repositoryVersion);
    }
  });
...
function scriptApplier(type, script, repositoryVersion) {
  mainUpdateButton.onclick = () => {
    ...
    mainUpdateButton.textContent =
      type === "update" ? "Updating..." : type === "install" ? "Installing..." : "Reinstalling...";
    mainUpdateButton.disabled = true;
    updateSteps.classList.add("main__update-steps--active");

    applyScript(repositoryScript)
      .then(() => {
        if (type !== "reinstall") window.localStorage.setItem("currentVersion", repositoryVersion);
        ...
        setTimeout(() => {
          ...
          mainUpdateButton.textContent =
            type === "update" ? "Updated!" : type === "install" ? "Installed!" : "Reinstalled!";
